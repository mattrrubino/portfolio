...
export default function ProjectContainer({ innerRef, proj, venue, desc, tech, link, desktop, media }) {
    ...
    <Container>
      <div ref={innerRef} className="h-full flex flex-col p-8 text-2xl md:text-4xl lg:text-5xl">
        ...
        <div className="flex-1 flex flex-col lg:flex-row lg:space-x-32">
          <div className="flex flex-col justify-center leading-none text-xl md:text-3xl lg:text-4xl lg:flex-1 lg:mb-16">
            {desc.map((line, i) => <span key={i} className="leading-none my-2 lg:my-4" dangerouslySetInnerHTML={{__html: line}} />)}
          </div>
          <div className="flex-1 flex flex-col justify-center items-center">
            <MediaRenderer media={media} />
            {(link && (!desktop || window.innerWidth >= 1024)) ? <button className="p-2 my-4 w-fit text-4xl bg-accent" onClick={() => navigate(link)}>Check It Out</button> : <div className="my-4 lg:my-10"></div>}
          </div>
        </div>
